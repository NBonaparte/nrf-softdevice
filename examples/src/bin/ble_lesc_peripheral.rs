...
impl SecurityHandler for Bonder {
    ...
    fn compare_passkey(&self, reply: NumericComparisonReply) {
        // In a real application, there should be some input that allows the user to confirm or
        // deny that the passkeys on both devices are equal
        unwrap!(reply.reply(true))
    }
    ...
    fn calc_public_key(&self, secret: &[u8; 32]) -> [u8; 64] {
        debug!("calculating public key for secret {:?}", secret);
        let secret = SecretKey::from_bytes(secret).unwrap();
        let mut pub_key: [u8; 64] = secret.public_key().to_untagged_bytes();
        // reverse the x and y byteorder separately
        pub_key[..32].reverse();
        pub_key[32..].reverse();
        pub_key
    }

    fn calc_shared_secret(&self, own_secret: &[u8; 32], peer_public_key: &[u8; 64]) -> [u8; 32] {
        debug!("calculating secret");
        let secret = SecretKey::from_bytes(own_secret).unwrap();
        let mut p_pk: [u8; 64] = *peer_public_key;
        p_pk[..32].reverse();
        p_pk[32..].reverse();
        let peer_public_key = PublicKey::from_untagged_bytes(&p_pk).unwrap();
        let mut shared_secret: [u8; 32] = *secret.agree(&peer_public_key).as_bytes();
        // output is big endian, so we need to reverse the bytes
        shared_secret.reverse();
        debug!("finished calculating secret");
        shared_secret
    }
}
...
impl BatteryService {
    pub fn new(sd: &mut Softdevice) -> Result<Self, RegisterError> {
        ...
        let attr = Attribute::new(&[0u8]).security(SecurityMode::LescMitm);
